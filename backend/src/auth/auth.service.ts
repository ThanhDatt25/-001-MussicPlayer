...
export class AuthService {
  ...
  async loginWithThirdParty(SocialUserDto: SocialUserDto) {
    const checkUser = await this.usersService.findOneByUsername(
      SocialUserDto?.email,
    );
    if (!checkUser) {
      const newUser = await this.usersService.registerGithubAccount(
        SocialUserDto,
      );
      const payload = {
        sub: 'Token login',
      ...
      const refresh_token = this.createRefreshToken(payload);
      const a = await this.usersService.updateUserToken(
        refresh_token,
      ...
      const a = await this.usersService.updateUserToken(
        refresh_token,
        checkUser?._id as unknown as string,
      );
      return {
        access_token: this.jwtService.sign(payload),
        refresh_token: refresh_token,
  ...
  async checkCode(data, user: IUser) {
    ...
    const verify = await this.verifyModel
      .findOne({ email: user?.email, code: code })
      .sort({ createdAt: -1 });
    if (verify) {
      const currentTime = new Date();
      const codeCreatedTime = new Date(verify.createdAt);
      const timeDifference =
        (currentTime.getTime() - codeCreatedTime.getTime()) / 1000 / 60; // Chuyển đổi từ milliseconds sang minutes
