...
export class MailController {
  constructor(
    ...
    private readonly followerModel: SoftDeleteModel<FollowerDocument>,
  ...
  async handleTestEmail() {
    ...
    for (const fols of followers) {
      ...
      const tracksWithMatchingAuthors = await this.trackModel
        .find({
          user: { $in: authors },
        })
      ...
      if (tracksWithMatchingAuthors?.length) {
        const tracks = tracksWithMatchingAuthors.map((track) => {
          return {
            ...
            photo:
              this.configService.get<string>('RENDER_BE') +
              'images/' +
              track?.photo,
          };
        });
