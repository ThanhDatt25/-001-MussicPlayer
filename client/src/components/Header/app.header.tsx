...
const AppHeader = () => {
    ...
    return (
        <div className="bg-black h-[70px] flex justify-between">
            <section className="flex space-x-4 ml-[35px]">
                ...
                <div className="flex items-center justify-center " onClick={() => { handleRedirectHome() }}>
                    <Button className="w-12 h-12 rounded-full hover:bg-[#8a7373] transition-all">
                        <svg className='h-8 w-8 fill-white' data-encore-id="icon" role="img" aria-hidden="true" viewBox="0 0 24 24"><path d="M13.5 1.515a3 3 0 0 0-3 0L3 5.845a2 2 0 0 0-1 1.732V21a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1v-6h4v6a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V7.577a2 2 0 0 0-1-1.732l-7.5-4.33z"></path></svg>
                    </Button>
                </div>
                <div className="flex items-center w-[900px]">
                    <div className="relative w-full max-w-md">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-white" size={20} />
                        <Input
                            type="text"
                            placeholder="What do you want to play?"
                            className="pl-10 pr-4 py-2 font-bold border rounded-full w-full focus:ring-2 focus:ring-white focus:border-white border-none bg-[#1f1f1f] text-white h-[45px]"
                            onKeyDown={(e: any) => {
                                if (e.key === "Enter") {
                                    if (e?.target?.value)
                                ...
                                {session?.user?.type === 'CREDENTIAL' ? (
                                    <DropdownMenu>
                                        <DropdownMenuTrigger>
                                            <img
                                                src={session?.user?.avatar && session?.user?.avatar !== "" ?
                                                    (session?.user?.type === 'CREDENTIAL' ? `${process.env.NEXT_PUBLIC_BACKEND_PUBLIC}${session?.user?.avatar}` : session?.user?.avatar)
                                                    : "/defaultuser.jpg"}
                                        ...
                                        <DropdownMenuContent align="end" className="bg-[#282828] border-none w-[230px]">
                                            ...
                                            <DropdownMenuItem className="flex p-3 hover:!bg-[#3e3e3e] hover:cursor-pointer transition-all">
                                                <ActiveLink href="/track/upload">
                                                    <div className="text-white w-full text-center text-[14px]">Upload</div>
                                                </ActiveLink>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="flex p-3 hover:!bg-[#3e3e3e] hover:cursor-pointer transition-all">
                                                <ActiveLink href="/likes">
                                                    <div className="text-white w-full text-center text-[14px]">Likes</div>
                                                </ActiveLink>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="flex p-3 hover:!bg-[#3e3e3e] hover:cursor-pointer transition-all">
                                                <ActiveLink href="/playlist">
                                                    <div className="text-white w-full text-center text-[14px]">PlayLists</div>
                                                </ActiveLink>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="flex p-3 hover:!bg-[#3e3e3e] hover:cursor-pointer transition-all">
                                                <ActiveLink href="/playlist">
                                                    <div className="text-white w-full text-center text-[14px]">Upgrade to premium</div>
                                                </ActiveLink>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="flex p-3 hover:!bg-[#3e3e3e] hover:cursor-pointer transition-all">
                                                <div className="text-white w-full text-center text-[14px]" onClick={
                                                    () => {
                                                        handleMenuClose();
